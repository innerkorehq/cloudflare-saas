...
// Initialize cache with env-based size
let hostCache;

async function resolveTenantFromHost(host, env) {
  // Initialize cache on first use
  if (!hostCache) {
    const cacheSize = parseInt(env.CACHE_MAX_SIZE || '1000', 10);
    hostCache = new LRUCache(cacheSize);
  }
  ...
  // Slow path: query D1 database for custom domains
  try {
    const defaultZone = env.DEFAULT_ZONE || 'default';
    
    // Query D1 database with composite key (name, zone)
    ...
    if (result && result.tenant_id) {
      hostCache.set(host, result.tenant_id);
      return result.tenant_id;
    }
  } catch (error) {
    console.error('Failed to resolve custom domain from D1:', error);
  }
...
async function handleRequest(request, env) {
  ...
  // Try multiple paths for Next.js static export (App Router)
  // Pattern: try $uri, $uri.html, $uri/index.html
  const keysToTry = [];
  
  if (key.endsWith('.html')) {
    // Already has .html extension, try as-is
    keysToTry.push(key);
  } else if (key.endsWith('/')) {
    // Ends with slash, try index.html
    keysToTry.push(`${key}index.html`);
    keysToTry.push(key.slice(0, -1) + '.html');
  } else {
    // No extension, try multiple patterns
    keysToTry.push(key); // Exact match (for assets)
    keysToTry.push(`${key}.html`); // Next.js page pattern
    keysToTry.push(`${key}/index.html`); // Directory index
  }
  ...
  try {
    ...
    // Final fallback: try root index.html for SPA client-side routing
